...
class OwnerService {
    ...
    func getAllRequestPosts(completion: @escaping (_ success: Bool, _ message: String, _ walkers: [RequestPost
                                                                                           ]) -> ()) {
        ...
        AF.request(url, method: .get, parameters: nil, encoding: URLEncoding.default, headers: nil, interceptor: nil).response { (responseData) in
            ...
            do {
                let walkerResponse = try JSONDecoder().decode(ApiResponse<[RequestPost]>.self, from: data)
                completion(walkerResponse.isSuccess, walkerResponse.message, walkerResponse.data ?? [])
            } catch {
                print("error", error)
                completion(false, "Something went wrong",[])
